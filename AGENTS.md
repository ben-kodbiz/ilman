...
- The regression suite is the gate for model/routing changes: run `scripts.run_regression` after any routing/prompt/validator change; hallucination rate must not regress. Run `scripts.run_companion_eval` after any companion/safety change; crisis routing must stay 100%.
- Companion layer rules (fix_me.md): empathy first, never dump religion on emotional statements (route `quran_question` etc. to RAG instead); at most ONE follow-up question; crisis signals short-circuit to the canned model-free safety response — never continue companion mode, never religious guilt; transient emotions are conversation state, NEVER memory (§5C gate in `CompanionMemory.save_fact`).
...
- Companion chat logs (`knowledge/processed/companion_logs/`, gitignored): all user+companion turns are captured as structured JSONL for owner troubleshooting — local-only, never published, no chain-of-thought (§31), crisis turns flagged `sensitive` (exports default to excluding them; redaction via `DELETE /api/v1/logs/{session}` or `--redact`). Eval/test session ids (`case-`, `scen-`, `eval-`, `s1`-`s6`, `adv-`, `v3-`, `dua-test`, `pillar-`, `fatihah-`) never log. Analyze: `uv run python -m scripts.companion_logs stats|sessions|read|export|export-all|watch`.
